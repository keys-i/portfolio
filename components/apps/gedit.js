...
export class Gedit extends Component {
  ...
  sendMessage = async () => {
    ...
    if (!serviceID || !templateID || !publicKey) {
      this.setState({ error: "The contact form is not configured yet." });
      return;
    }
    ...
    try {
      ...
      this.setState({ error: "The message could not be sent. Please try again." });
    } finally {
    ...
    return (
      <div className="w-full h-full relative flex flex-col bg-ub-cool-grey text-white select-none">
        ...
        <div className="relative flex-grow flex flex-col bg-ub-gedit-dark font-normal windowMainScreen">
          ...
          {this.state.error ? (
            <p className="px-6 py-1 text-sm text-red-300" role="alert">
              {this.state.error}
            </p>
          ) : null}
        </div>
