...
export class Terminal extends Component {
  ...
  terminalRow = (id) => {
    return (
      <React.Fragment key={id}>
        ...
        <div id={`row-result-${id}`} className="my-2 whitespace-pre-wrap font-normal"></div>
      </React.Fragment>
    );
  };
  ...
  childDirectories = (parent) => {
    return this.child_directories[parent].map((file) => `'${file}'`).join("  ");
  };
  ...
  handleCommands = (command, rowId) => {
    ...
    switch (main) {
      ...
      case "exit":
        ...
        return;
      case "sudo":
        result = "Nice try. Sudo access is not available here.";
        break;
      default:
        result =
          "Command '" +
